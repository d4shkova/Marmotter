...
The description above is the default theme, **Midnight**. Fourteen more ship
beside it — Monochrome, Ember, Blossom, Paper, Nebula, the seven named in
French (Menthe, Lagune, Néon, Sapin, Crépuscule, Orage, Brume), and the two
dark twins, Brume Nuit and Paper Dark — and each is exactly what this section
promises a theme is: a block of primitives under `[data-theme='…']` in
...
A dark twin is a new palette rather than an inverted one, because inverting a
theme end for end gets a different theme than the one it is named after: the
pale steps a light theme writes its text in are its saturated ones, so Brume
swapped that way is a navy theme rather than a faded one. Each is built from
its own end instead and held to the same floor as everything else.

Which is what the picker shows. A theme's row carries a working miniature of
the window — sidebar, conversation with names in it, composer — drawn by
putting `data-theme` on the frame and reading the same aliases the window
reads, so a preview cannot drift from what it previews
(`packages/ui/src/app/ThemePreview.tsx`). Three chips said which colours a
theme had; they could not say whether a channel was readable in it, which is
the question somebody choosing a theme is actually asking.
...
Both are settings, and the two roles are chosen separately because they are not
interchangeable: seven interface faces and seven message faces in
`packages/ui/src/fonts.ts`, applied the way a theme is — one custom property on
the root element, which the whole stylesheet resolves its type through, with no
component branching on a font. Two rules hold. Every message face is
monospaced, which is not a style preference: the nick column is measured in
characters and the raw log is read by lining fields up, both of which a
proportional face silently breaks. And nothing is downloaded — a webfont
fetched at startup tells an arbitrary host the user's address, which is the
trade link unfurling makes and gets the same answer — so each entry names faces
the platforms already install and ends in its generic family, and a machine
with none of them lands on its own default.
...
| `DCC` / `XDCC` | **Receive-only. Desktop and Android; never web.** A file monitor (off by default, enabled under Settings → User options) lists files in a panel and downloads them on request. The panel is a frame with one scrolling list in it, not a page that scrolls: the search, the downloads tray, the download folder and the column headers are rows of a flex column, so none of them can be scrolled away from at any window size. Sticky would have read the same until the window was resized — a sticky header belongs to a scroll position, so a window made shorter keeps its offset and the headings end up somewhere in the middle of the list. It sees files two ways: a direct `DCC SEND`, and XDCC pack advertisements — the `#N Ngx [size] name` catalogue lines serving bots post in a channel (`packages/protocol/src/xdcc.ts`). Downloading a pack sends `XDCC SEND #N` to the bot; its answering `DCC SEND` (`packages/protocol/src/dcc.ts`) is matched back to that row and fetched. Two things come between those, because in practice they are what decides whether a file arrives. A bot answers a request with a notice long before it answers with a file — a queue position, or a refusal — so those are read (`parseXdccResponse`) and turned into the row's own words (`applyXdccResponse`): a queue position keeps it waiting, "you must be on a known channel" fails it with the thing to do about it, and a request nobody is waiting for any more is withdrawn rather than left in the bot's queue to be answered into nothing — `XDCC REMOVE` for a queue place, `XDCC CANCEL` for a transfer the bot has already opened, which is also sent before a retry, since a bot holding a pending transfer answers the next request with "you have a DCC pending" until its own timeout. A pack is asked for by typing the message at least as often as by pressing the button — every index prints the message, not a button — so an outgoing `xdcc send #N` is recognised wherever it was typed and registered as the same request, and an offer for a listed row that nothing here started is announced rather than fetched on the sender's say-so or dropped in silence. And a pack request is more often pasted than browsed — every index on the web hands out an `irc://` link and a literal `/msg Bot xdcc send #N` — so `parseXdccRequest` reads that pair, the link picks the network among those connected and joins the channel it names, and the row that results behaves from then on like one the monitor saw advertised. That paste is taken by `/xdcc` in the command bar rather than by a field in the file monitor: a box asking for text of an unstated shape is a box only somebody who already knew the answer can fill in, and the command bar is the one surface that documents what it takes as it is being typed. A transfer with no bytes yet says where it is connecting, because a receiver answers a direct offer on the socket rather than over IRC — there is nothing in the raw log to show it is happening, and the address is what a transfer that never starts turns on. An offer whose address is one only the sender's own network can reach is still attempted — the receiver may be on that network — and when that fails the same port is tried once at the sender's own host, from their hostmask on IRC (`publicAddressFor`). That is the common failure in practice: a serving bot behind a router that has not been told its public address advertises the one it knows, so the port is real and only the address is wrong, and the bot's file server is usually the machine the bot is already being reached at. The fallback is taken in that order and never the other way round, because a receiver who *is* on the sender's network can use the advertised address and that attempt should win where it can; it is taken once rather than in a loop, since the second failure is the answer; and it is offered only for an address that cannot route, so a transfer that failed for some other reason is never retried elsewhere and then blamed on the address. A cloak is not an address — it resolves to nothing — so there is no fallback behind one, and recognising one is more than looking for a slash: `Rizon-B5D54D46.cust.smartspb.net` splices a network name and a hash over the real leading label and is otherwise an ordinary hostname, and `863933A7.7304A9F.C6F98C0D.IP` is hex under a pseudo-domain. Both were dialled as addresses once, which spends a lookup that cannot succeed and then blames a hostname the network invented. The check errs towards refusing, because a real hostname mistaken for a cloak costs one fallback nobody was promised while the reverse costs a wait and a wrong explanation. The row keeps what was advertised separately from where it is currently dialling, so Retry asks for the whole sequence again rather than starting from the fallback. When both fail the row says so and names the remaining option, another bot, rather than reporting a refused connection: no amount of retrying fixes somebody else's router, and "connection refused" sends a person looking at their own firewall for something that is not there. A failure is also where the bot gets its slot back. A serving bot opens one transfer at a time and holds it for its own timeout — three minutes is usual — so a transfer that has failed here is a slot nobody will connect to, and until the bot is told, every further request bounces off it with "you can only have 1 transfer at a time" and is queued behind the dead one. Withdrawing on a final failure is what the bot's own reminder asks for, and it is the difference between the next attempt going out now and going out three minutes from now. Only on a final failure: the fallback attempt still needs the slot the bot is holding. A request no bot ever answers gives up after fifteen minutes of total silence and says so, rather than spinning at "Requested" for the rest of the session with only the bin to press — the two HexChat add-ons this was compared against both leave that row spinning, and one leaks a concurrency slot doing it. The clock is reset by anything the bot says, because a bot narrating a queue position has plainly heard us and a queue place of half an hour is ordinary; only silence counts. Nothing is withdrawn when it fires, because a bot that never spoke probably never queued us, and an answer that arrives later is matched back to the row and fetched exactly as it would have been. A bot that hands out an unroutable address hands out the same one to everybody, so one failed transfer condemns everything else it is offering: the rest of that sender's rows say so before anybody spends three minutes finding out again, while keeping the Download button, since the reader may be on that network or the owner may have fixed it since. It is derived from the rows rather than recorded, because a failed row already keeps the address that was advertised — which also means clearing the list clears the verdict, as an observation about this session rather than a mark against the bot for ever. An offer that claims to be a file and cannot be read as one is listed as a failed row carrying the raw line rather than dropped, because a person who asked for a file and got nothing cannot otherwise tell an unanswered request from an answer this client did not understand. The socket-to-file download is `crates/marmotter-transport/src/dcc.rs`, behind a Tauri command shared by both shells (`crates/marmotter-shell/src/dcc.rs`), and it reads the offer's variant rather than assuming the plain one: an `SSEND` is a TLS socket and is dialled as one — the certificate cannot be verified against an address and is not, so what the handshake buys is confidentiality in transit — and a `TSEND` sender never reads its socket, so the four-byte acknowledgements are omitted for it rather than filling its window until our write blocks. Both were previously dialled as ordinary sends, which fails in the way that looks exactly like a firewall. The two shells differ only around it: desktop picks the download folder with a dialog and can reveal a saved file in the file manager, and Android does neither — an app there writes inside its own storage or asks for a permission that would let it read the whole device, and no file manager will open that storage — so the shell names the folder itself and the reveal button is absent rather than drawn and broken. Web has no file monitor at all: a browser tab has no folder and cannot open the direct socket. A transfer that broke part-way is continued rather than started again: bytes go to a `.part` file that survives a failure (a cancel discards it — stopping a download is somebody saying they do not want it), and the next attempt asks the sender to resume from what is there (`buildDccResume`) and begins at the position the sender names back (`parseDccAccept`), which is not always the one asked for. A sender that never answers is not one that refused: the file simply starts again. That deadline is absolute, and a re-offer joins the attempt in flight rather than restarting it, which is the difference between the transfer working and the transfer never happening at all. A serving bot re-offers a pack every few seconds until somebody connects; when each re-offer began the negotiation again it also replaced the deadline, so a bot re-offering every five seconds against an eight-second deadline meant the deadline could never arrive — the client asked to resume thirty-six times, the bot held a socket nobody dialled, and three minutes later it closed it with “Closing Connection: DCC Timeout”. Nothing about that looked wrong on screen, which is why it survived: every piece worked on its own and the fault was in how they met over time. A re-offer therefore contributes only its address, since a bot may re-offer on a new port and that is the one it is now listening on. For the same reason every answer a dial waits on has a deadline of its own: a shell that never says what is on disk starts the file from the beginning rather than stopping the transfer, because an unresumed file is worth far more than an unstarted one. Passive (reverse) offers are received too, because they are how a sender that cannot be connected to hands over a file, and refusing them made a whole class of bot unusable for reasons the interface then had to describe as somebody else's fault. There the socket is ours: the shell binds one, the reply carries the port and the offer's token back (`buildPassiveAccept`), and only the sender's own address is accepted on it — the port is advertised in a message anybody may be reading, and the alternative is that the first stranger to dial it decides what lands in the download folder. It needs an address the sender can reach, which the shell reads off the route to them and which behind a router is not the one this machine knows about, so Settings can name the address to give out and a transfer with neither says so rather than timing out. **Sending and DCC CHAT remain out of scope**; their security surface is disproportionate to value. |
...
2. **Raw log tab.** Per-network, showing the full bidirectional line stream with
   send/receive direction, timestamps, filter, and copy. Control characters are shown as their Unicode
   pictures rather than rendered invisibly: a `DCC SEND` and somebody typing
   the words "DCC SEND" are otherwise the same line on screen, which is the
   distinction this tab is most often opened to settle. This is how a power
